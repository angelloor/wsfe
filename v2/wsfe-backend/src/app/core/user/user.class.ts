			...
			if (fs.existsSync(initialPath)) {
				...
				await dml_user_upload_avatar(this, newAvatarPath)
					.then((response: any) => {
						...
						if (!fs.existsSync(this.avatar_path_base)) {
							fs.mkdir(this.avatar_path_base, (error) => {
								if (error) {
						...
						fs.rename(
							`./avatar.jpg`,
							`${this.avatar_path_base}/${newAvatarPath}`,
							(err) => {
								if (err) {
									reject(err);
								} else {
