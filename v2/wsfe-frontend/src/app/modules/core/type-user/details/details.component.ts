...
export class TypeUserDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._typeUserService.typeUser$
      ...
      .subscribe((typeUser: TypeUser) => {
        ...
        if (this.typeUser.name_type_user === 'Consulta') {
          this.typeUserForm.get('name_type_user')?.disable();
        } else {
          this.typeUserForm.get('name_type_user')?.enable();
        }
  ...
  updateTypeUser(): void {
    ...
    typeUser = {
      ...
      company: {
        id_company: parseInt(typeUser.company.id_company),
      },
      profile: {
        ...
        next: (_typeUser: TypeUser) => {
          if (_typeUser) {
            this._notificationService.success(
              'Tipo de usuario actualizada correctamente'
            );
            /**
             * Toggle the edit mode off
             */
  ...
  deleteTypeUser(): void {
    this._angelConfirmationService
      .open({
        ...
        message:
          '¿Estás seguro de que deseas eliminar esta tipo de usuario? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((confirm: ActionAngelConfirmation) => {
        if (confirm === 'confirmed') {
           ...
           */
          this._typeUserService
            .deleteTypeUser(id_user_, id_type_user)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe({
              next: (response: boolean) => {
                if (response) {
                   ...
                   */
                  this._notificationService.success(
                    'Tipo de usuario eliminada correctamente'
                  );
                  /**
