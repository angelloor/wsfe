    ...
    this.searchInputControl.valueChanges
      .pipe(
        ...
        switchMap((query) => {
           ...
           * Search
           */
          return this._typeUserService.readTypeUserByQuery(query.toLowerCase());
        })
      )
      .subscribe();
    /**
  ...
  createTypeUser(): void {
    this._angelConfirmationService
      .open({
        ...
        message:
          '¿Estás seguro de que deseas añadir una nueva tipo de usuario? ¡Esta acción no se puede deshacer!',
      })
      .afterClosed()
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((confirm: ActionAngelConfirmation) => {
        if (confirm === 'confirmed') {
          ...
          /**
           * Create the tipo de usuario
           */
          this._typeUserService
            .createTypeUser(id_user_)
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe({
              next: (_typeUser: TypeUser) => {
                if (_typeUser) {
                  this._notificationService.success(
                    'Tipo de usuario agregada correctamente'
                  );
                  /**
                   * Go to new tipo de usuario
                   */
