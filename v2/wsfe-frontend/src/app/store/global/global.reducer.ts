...
const _sign_in_visitor: boolean = false;
...
const _rememberMeVisitor: RememberMeVisitor = {
  enabled: false,
  buyer_identifier_voucher: '',
};

const _environment_test: TYPE_ENVIRONMENT = '1';
const _environment_production: TYPE_ENVIRONMENT = '2';
...
export const globalState: AppInitialData = {
  ...
  inactive: _inactive,
  inside: _inside,
  sign_in_visitor: _sign_in_visitor,
  rememberMe: _rememberMe,
  rememberMeVisitor: _rememberMeVisitor,
  query_environment: _environment_production,
  user: _user,
...
const _reducer = createReducer(
  ...
  // SignInVisitor
  on(setSignInVisitor, (state) => {
    return {
      ...state,
      sign_in_visitor: true,
    };
  }),
  on(resetSignInVisitor, (state) => {
    return {
      ...state,
      sign_in_visitor: _inside,
    };
  }),
  // RememberMe
  ...
  // RememberMeVisitor
  on(enabledRememberMeVisitor, (state, _rememberMeVisitor) => {
    return {
      ...state,
      rememberMeVisitor: {
        enabled: _rememberMeVisitor.enabled,
        buyer_identifier_voucher: _rememberMeVisitor.buyer_identifier_voucher,
      },
    };
  }),
  on(disabledRememberMeVisitor, (state) => {
    return {
      ...state,
      rememberMeVisitor: _rememberMeVisitor,
    };
  ...
  // query_environment
  on(enabledEnvironmentTest, (state) => {
    return {
      ...state,
      query_environment: _environment_test,
    };
  }),
  on(enabledEnvironmentProduction, (state) => {
    return {
      ...state,
      query_environment: _environment_production,
    };
